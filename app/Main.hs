...
instance FromJSON Purify where
  parseJSON j = do
    ...
    pure (Purify {..})
    where
      flattenDeps :: Map String Dep -> [Dep]
...
purify inputFiles config fileWatch verbose = do
  ...
  when (extraDepMismatch || extraDepNameMismatch)
    ...
    let clone =
        ...
        checkout = do
          tags <-
            fmap
              lines
              (readProcess "git" ["-C", gitDir, "tag", "--points-at", "HEAD"] "")
        ...
        concat
          (mapMaybe
             (\dep -> do
                ...
                pure
                  (map (\modn -> getDepDir dep ++ "/" ++ topath modn) modules))
             (extraDeps config))
        where
...
withErrorMsg :: String -> ExitCode -> IO a -> IO a
